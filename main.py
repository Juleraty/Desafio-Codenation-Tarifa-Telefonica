    ...
    for record in records:
        ...
        for result in results:
            if result['source'] == record['source']:
                i = 1
                previus_value = result['total']
                price =  call_price(record['start'], record['end'])
                price_update = round((previus_value + price), 2)
                result['total'] = price_update

        if i == 0:
            price = call_price(record['start'], record['end'])
            price_rounded = round(price, 2)
            results.append({'source': record['source'], 'total': price_rounded})
    final_results = sorted(results, key=lambda result: result['total'], reverse=True)
